(() => {
  const data = window.EMA_SHEETS;

  if (!data) {
    document.body.classList.add("site-error");
    document.body.insertAdjacentHTML(
      "afterbegin",
      "<p class=\"error-banner\">EMA sheet data did not load.</p>"
    );
    return;
  }

  const route = window.VACCINE_SITE_ROUTE || {};
  if (route.section !== "ema") {
    return;
  }
  ...
  if (!sheet) {
    ...
    document.body.insertAdjacentHTML(
      "afterbegin",
      "<p class=\"error-banner\">No EMA vaccine sheets are available.</p>"
    );
    return;
  }
  ...
  const table = document.getElementById("ema-sheet-table");
